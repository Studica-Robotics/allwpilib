...
static bool AllocateVMXPiDIO(std::shared_ptr<DigitalPort> digPort, bool input, int32_t* status) {
	...
	if (input) {
		if (raspi::vmxIO->ChannelSupportsCapability(digPort->vmx_chan_info.index, VMXChannelCapability::DigitalInput)) {
		...
		if (raspi::vmxIO->ChannelSupportsCapability(digPort->vmx_chan_info.index, VMXChannelCapability::DigitalOutput)) {
	...
	if (!raspi::vmxIO->ActivateSinglechannelResource(vmx_chan_info, &digPort->dio_config, digPort->vmx_res_handle, status)) {
		return false;
	}
...
static void DeallocateVMXPiDIO(std::shared_ptr<DigitalPort> digPort, bool& isActive) {
	isActive = false;
	raspi::vmxIO->IsResourceActive(digPort->vmx_res_handle, isActive, raspi::vmxError);
	if (isActive) {
		raspi::vmxIO->DeallocateResource(digPort->vmx_res_handle, raspi::vmxError);
	}
...
extern "C" {
    ...
    HAL_DigitalPWMHandle HAL_AllocateDigitalPWM(int32_t* status) {
        ...
        // Raspi Note:  Each DIO channel with "output capability" may
     ...
     * logarithmic (this sentence applies to athena, not Raspi).
     *
    ...
    void HAL_SetDIO(HAL_DigitalHandle dioPortHandle, HAL_Bool value, int32_t* status) {
        ...
        if (EXTRACT_VMX_RESOURCE_TYPE(port->vmx_res_handle) == VMXResourceType::PWMGenerator) {
        	*status = RASPI_DIO_ASSIGNED_TO_PWMGEN;
        	return;
        }

        uint8_t retry_count = 3;
        do { 
                raspi::vmxIO->DIO_Set(port->vmx_res_handle, value ? true : false, status);
    ...
    HAL_Bool HAL_GetDIO(HAL_DigitalHandle dioPortHandle, int32_t* status) {
        ...
        bool high = true;
        raspi::vmxIO->DIO_Get(port->vmx_res_handle, high, status);
    ...
    void HAL_Pulse(HAL_DigitalHandle dioPortHandle, double pulseLength,
        ...
        do { 
                raspi::vmxIO->DIO_Pulse(port->vmx_res_handle, true /*high*/, num_microseconds, status);
        } while (RetryWriteOnBoardCommError(status, retry_count));         
        return;
    }
    ...
    HAL_Bool HAL_IsPulsing(HAL_DigitalHandle dioPortHandle, int32_t* status) {
        ...
        raspi::vmxIO->DIO_IsPulsing(port->vmx_res_handle, is_pulsing, status);
        ClearBoardCommErrorStatus(status);
    ...
    HAL_Bool HAL_IsAnyPulsing(int32_t* status) {
        ...
        raspi::vmxIO->DIO_GetNumPulsing(num_pulsing);
        ClearBoardCommErrorStatus(status);        
