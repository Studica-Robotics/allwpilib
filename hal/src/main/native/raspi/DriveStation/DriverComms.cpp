...
#include "include/RaspiDriveData.h"
#include "RaspiTime.h"
...
namespace raspi {
	...
	volatile uint8_t	robotMode = RASPI_COMMS_STATE_DISABLED;
...
uint32_t raspi::comms::getTotalSystemMemory() {
...
uint64_t raspi::comms::getAvailableDiskSpace() {
	struct statvfs stat;
	if (statvfs("/", &stat) != 0) {
		return 0;
	}
...
void raspi::comms::setShutdownHandler(void (*shutdown_handler_func)(int)) {
	raspi::comms::shutdown_handler = shutdown_handler_func;
}

void raspi::comms::start_log_capture() {
...
void raspi::comms::start_ds_protocol_threads() {
    if (!isRunning) {
...
void raspi::comms::encodePacket(char* data) {
    ...
    if (robotMode != RASPI_COMMS_STATE_DISABLED) {
    ...
    if (robotMode == RASPI_COMMS_STATE_AUTONOMOUS) {
	data[3] |= UDP_DS_STATUSBYTE_AUTONOMOUS;
    }
    if (robotMode == RASPI_COMMS_STATE_TEST) {
    ...
    if (robotMode == RASPI_COMMS_STATE_TELEOP) {
...
void raspi::comms::setInputVoltage(double new_voltage) {
	voltage = new_voltage;
}

void raspi::comms::setCANStatus(float percentBusUtilization, uint32_t busOffCount, uint32_t txFifoFullCount, uint8_t rxErrorCount, uint8_t txErrorCount)
{
	raspi::comms::canStatusPercentBusUtilization = percentBusUtilization;
	raspi::comms::canStatusBusOffCount = busOffCount;
	raspi::comms::canStatusTxFifoFullCount = txFifoFullCount;
	raspi::comms::canStatusRxErrorCount = rxErrorCount;
	raspi::comms::canStatusTxErrorCount = txErrorCount;
}

void raspi::comms::setRobotState(uint8_t mode /*RASPI_COMMS_STATE_xxx*/)
{
	raspi::comms::robotMode = mode;
}

void raspi::comms::setRobotBrownoutProtectionActive(bool brownout_protection_active)
{
	raspi::comms::robotBrownoutProtectionActive = brownout_protection_active;
}

void raspi::comms::setNotUserCode(bool not_user_code)
{
	raspi::comms::notUserCode = not_user_code;
}

void raspi::comms::setRobotESTOPActive(bool estop_active)
{
	raspi::comms::robotESTOPActive = estop_active;
}

void raspi::comms::setRobotProgramStarted(bool program_started)
{
	raspi::comms::robotProgramStarted = program_started;
}

//// ----- DriverStation Comms: Decode ----- ////

int32_t raspi::comms::enqueueErrorMessage(uint16_t num_occur, int32_t errorCode, uint8_t flags, const char *details, const char *location, const char *callStack) {
	ErrorOrPrintMessage m;
...
int32_t raspi::comms::enqueuePrintMessage(char *msg) {
	ErrorOrPrintMessage m;
	// Calculate required length
...
void raspi::comms::decodeTcpPacket(char* data, int length) {
	...
	while (currTagDataFirstIndex < length) {
		...
		switch (currTag) {
			case DS_TCP_PROTOCOL_TAG_JOYSTICK_DESCRIPTIONS:
			{
				...
				while ((i - currTagDataFirstIndex) < currTagDataLen) {
					...
					if (type != 255 && axis_count <= HAL_kMaxJoystickAxes) {
						...
						Raspi_DriveData::updateJoyDescriptor(joyid, &desc);
					}
			...
			case DS_TCP_PROTOCOL_TAG_NEWER_MATCH_INFO:
			{
				...
				Raspi_DriveData::updateMatchIdentifyInfo(event_name, match_type, match_number, replay_number);
				break;
			}
			case DS_TCP_PROTOCOL_TAG_GAME_SPECIFIC_MSG:
			{
				uint16_t game_specific_msg_size = currTagDataLen;
				Raspi_DriveData::updateMatchGameSpecificMessage(game_specific_msg_size, static_cast<uint8_t *>(static_cast<void *>(&data[currTagDataFirstIndex])));
				break;
			}
			default:
				break;
...
// returns RASPI_COMMS_SHUTDOWN_XXX
int raspi::comms::decodeUdpPacket(char* data, int length) {

	if (length < DS_UDP_PROTOCOL_HEADER_LENGTH) return RASPI_COMMS_SHUTDOWN_NONE;
	...
	if (new_version_data_requested) {
        ...
        int shutdown_code = RASPI_COMMS_SHUTDOWN_NONE;
        if (reboot || restart) {
        	if (reboot) {
        		shutdown_code = RASPI_COMMS_SHUTDOWN_REBOOT;
        		...
        		shutdown_code = RASPI_COMMS_SHUTDOWN_RESTART;
	    ...
	    setRobotESTOPActive(true);
            ...
            return RASPI_COMMS_SHUTDOWN_ESTOP;

        }
        HAL_AllianceStationID alliance = (HAL_AllianceStationID)data[DS_UDP_PROTOCOL_HEADER_INDEX_ALLIANCE_STATION_ID];

        bool ds_attached;
        if (raspi::vmxGetTime() - lastDsUDPUpdateReceivedTimestamp > DS_COMM_TIMEOUT_MICROSECONDS) {
            // DS Disconnected
            ds_attached = false;
        } else {
            // DS Connected
        ...
        while (i < length && search) {
            ...
            switch (tag) {
				case DS_UDP_PROTOCOL_TAG_JOYSTICK_STATE:
				{
					...
					for (int pv = 0; pv < joy->pov_count; pv++) {
						...
						bytes_for_pov += 2;
		            	    ...
		            	    Raspi_DriveData::updateJoyAxisUnsafe(joy_id, joy->axis_count, joy->axis);
				    Raspi_DriveData::updateJoyPOVUnsafe(joy_id, joy->pov_count, joy->pov);
				    Raspi_DriveData::updateJoyButtonsUnsafe(joy_id, joy->button_count, joy->button_mask);
				...
				case DS_UDP_PROTOCOL_MATCH_TIME_COUNTDOWN:
				{
					...
					Raspi_DriveData::updateMatchTimeUnsafe(matchTime);
					break;
				}
	...
	// Perform final update, release lock and signal that new data is available
        Raspi_DriveData::updateControlWordAndAllianceIDUnsafe(enabled, auton, test, eStop, fms, ds_attached, alliance);
	Raspi_DriveData::updateUnlockAndSignal();

	uint64_t now_time = raspi::vmxGetTime();
	//uint64_t delta = now_time - lastDsUDPUpdateReceivedTimestamp;
        lastDsUDPUpdateReceivedTimestamp = now_time;
    }
    return RASPI_COMMS_SHUTDOWN_NONE;
}
...
namespace raspi {
    namespace comms {
        ...
        void driverStationTCPDataReceivedCallback(int client_id, Toast::Net::Socket::ClientSocket sock) {
			...
			if (ret > 0) {
				raspi::comms::decodeTcpPacket(ds_tcp_rcv_buffer, ret);
			}
        }
	...
	void tcpProcess() {
		...
		printf("Error %d preparing DS Selective Server Socket!\n", ss_prepare_status);
            ...
            ds_sock.on_data(raspi::comms::driverStationTCPDataReceivedCallback);
	    ...
	    while (isRunning) {
    			...
    			try {
        			...
        			printf("raspi::comms::tcpProcess - Caught exception:  %s\n", ex.what());
        		}
			}
		...
		void udpProcess() {
			...
			while (true) {
				...
				try {
					...
					if (len > 0) {
						uint8_t shutdown_code = raspi::comms::decodeUdpPacket(ds_udp_rcv_buffer, len);
						raspi::comms::encodePacket(ds_udp_send_buffer);
...
#endif
						if (first_packet || (raspi::comms::udp_ds_control != last_udp_ds_control)) {
#if 0
							state_update = true;
#endif
							ds_ctl_string[0] = IS_BIT_SET(raspi::comms::udp_ds_control,7) ? '!' : '-';		  // E-Stop
							ds_ctl_string[1] = '.';
							ds_ctl_string[2] = '.';
							ds_ctl_string[3] = '.';
							ds_ctl_string[4] = IS_BIT_SET(raspi::comms::udp_ds_control,3) ? 'F' : '-';		  // FMS Present
							ds_ctl_string[5] = IS_BIT_SET(raspi::comms::udp_ds_control,2) ? 'E' : '-';		  // Enabled
							ds_ctl_string[6] = IS_BIT_SET(raspi::comms::udp_ds_control,1) ? 'A' : '-';           // Auto
							ds_ctl_string[7] = IS_BIT_SET(raspi::comms::udp_ds_control,0) ? 'X' : '-';		  // Test
							ds_ctl_string[8] = 0;							
						}
						if (first_packet || (raspi::comms::udp_ds_request != last_udp_ds_request)) {
#if 0
							state_update = true;
#endif
							ds_req_string[0] = '.';
							ds_req_string[1] = '.';
							ds_req_string[2] = '.';
							ds_req_string[3] = IS_BIT_SET(raspi::comms::udp_ds_request,4) ? 'P' : '-';            // Prog Start
							ds_req_string[4] = IS_BIT_SET(raspi::comms::udp_ds_request,3) ? 'H' : '-';            // Hard Restart
							ds_req_string[5] = IS_BIT_SET(raspi::comms::udp_ds_request,2) ? 'S' : '-';		   // Soft Restart
							ds_req_string[6] = IS_BIT_SET(raspi::comms::udp_ds_request,1) ? 'U' : '-';		   // Usage Request
							ds_req_string[7] = IS_BIT_SET(raspi::comms::udp_ds_request,0) ? 'V' : '-';		   // Version Request
							ds_req_string[8] = 0;							
						}
						...
						last_udp_ds_control = raspi::comms::udp_ds_control;
						last_udp_ds_request = raspi::comms::udp_ds_request;
						last_udp_robot_status = robot_status_code;
						last_udp_robot_program_trace = robot_program_trace;
						first_packet = false;

						if (shutdown_code != RASPI_COMMS_SHUTDOWN_NONE) {
						    ...
						    // Initiate program shutdown
					            if (shutdown_handler) {
					            	shutdown_handler(RASPI_COMMS_SHUTDOWN_RESTART);
					            } else {
						            stop();
						    }							
						}
						...
						// No data read.  Check to see if the ds is still attached.
					        if (raspi::vmxGetTime() - lastDsUDPUpdateReceivedTimestamp > DS_COMM_TIMEOUT_MICROSECONDS) {					            
					            Raspi_DriveData::updateDSAttached(false);
					        }
					}
				} catch(const std::exception& ex){
					printf("raspi::comms::udpProcess - Caught exception:  %s\n", ex.what());
        			}
			}
			sock.close();
		}
        		...
        		try {
					...
					if (ret == 0) {
						...
						if (FD_ISSET(out_pipe[0], &set)) {
							...
							// for line buffering in Raspi HAL Initialization.
							ssize_t num_read;
							...
							while ((num_read = getline(&line, &len, pipe_file)) != -1) {
								...
								raspi::comms::enqueuePrintMessage(line);
							}
						}
					} else {
						// Error
        			...
        			printf("raspi::comms::stdCaptureProcess - Caught exception:  %s\n", ex.what());
        		}
        	}
