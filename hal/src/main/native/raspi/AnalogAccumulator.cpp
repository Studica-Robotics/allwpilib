...
extern "C" {
    ...
    void HAL_ResetAccumulator(HAL_AnalogInputHandle analogPortHandle, int32_t* status) {
        ...
        raspi::vmxIO->Accumulator_Counter_Reset(port->vmx_res_handle, status);
    }
    ...
    void HAL_GetAccumulatorOutput(HAL_AnalogInputHandle analogPortHandle, int64_t* value, int64_t* count, int32_t* status) {
        ...
        raspi::vmxIO->Accumulator_Counter_GetValueAndCount(port->vmx_res_handle, accumulator_counter_value, accumulator_counter_count, status);
