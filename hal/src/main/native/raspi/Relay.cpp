...
extern "C" {
    HAL_RelayHandle HAL_InitializeRelayPort(HAL_PortHandle portHandle, HAL_Bool fwd, const char* allocationLocation, int32_t* status) {
        ...
        HAL_RelayHandle relay_handle;
        auto port = relayHandles->Allocate(wpi_relay_channel, &relay_handle, status);
        ...
        HAL_DigitalHandle digital_channel_handle =
        		HAL_InitializeDIOPort(createPortHandle(static_cast<uint8_t>(corresponding_wpi_dio_channel), module), input, allocationLocation, status);
