...
bool HAL_Internal_ActivatePWMGenerator(HAL_DigitalHandle pwmPortHandle, int32_t *status)
{
    ...
    if (!raspi::vmxIO->ActivateSinglechannelResource(vmx_chan_info, &port->pwmgen_config, port->vmx_res_handle, status)) {
		if (*status == VMXERR_IO_NO_UNALLOCATED_COMPATIBLE_RESOURCES) {
			*status = 0;
			VMXResourceHandle resourceWithAvailablePort;
			bool allocated;
			if (raspi::vmxIO->GetResourceHandleWithAvailablePortForChannel(
				PWMGenerator, port->vmx_chan_info.index, port->vmx_chan_info.capabilities, resourceWithAvailablePort, allocated, status)) {
				if (raspi::vmxIO->RouteChannelToResource(port->vmx_chan_info.index, resourceWithAvailablePort, status)) {
...
void HAL_FreePWMPort(HAL_DigitalHandle pwmPortHandle, int32_t* status) {
    ...
    raspi::vmxIO->UnrouteChannelFromResource(port->vmx_chan_info.index, vmxResource, status);
    port->vmx_res_handle = CREATE_VMX_RESOURCE_HANDLE(VMXResourceType::Undefined,INVALID_VMX_RESOURCE_INDEX);
    bool isActive = false;
    raspi::vmxIO->IsResourceActive(vmxResource, isActive, status);
    if (isActive) {
        bool allocated = false;
        bool isShared = false;
        raspi::vmxIO->IsResourceAllocated(vmxResource, allocated, isShared, status);
        if (allocated) {
			uint8_t num_routed_channels = 0;
			raspi::vmxIO->GetNumChannelsRoutedToResource(vmxResource, num_routed_channels, status);
			if (num_routed_channels == 0) {
					raspi::vmxIO->DeallocateResource(vmxResource, status);
			}
        }
    }
...
void HAL_SetPWMRaw(HAL_DigitalHandle pwmPortHandle, int32_t value, int32_t* status) {
    ...
    do { 
        raspi::vmxIO->PWMGenerator_SetDutyCycle(port->vmx_res_handle, portIndex, value, status); 
...
void HAL_SetPWMSpeed(HAL_DigitalHandle pwmPortHandle, double speed, int32_t* status) {
    ...
    uint8_t retry_count = 3;
    do { 
        raspi::vmxIO->PWMGenerator_SetDutyCycle(port->vmx_res_handle, portIndex, dutyCycle, status);
    } while (raspi::RetryWriteOnBoardCommError(status, retry_count));    
}
...
void HAL_SetPWMDisabled(HAL_DigitalHandle pwmPortHandle, int32_t* status) {
    ...
    uint8_t retry_count = 3;
    do { 
        raspi::vmxIO->PWMGenerator_SetDutyCycle(port->vmx_res_handle, portIndex, 0, status);
    } while (raspi::RetryWriteOnBoardCommError(status, retry_count));      
}
...
int32_t HAL_GetPWMRaw(HAL_DigitalHandle pwmPortHandle, int32_t* status) {
    ...
    raspi::vmxIO->PWMGenerator_GetDutyCycle(port->vmx_res_handle, portIndex, &currDutyCycleValue, status);
...
void HAL_SetPWMPeriodScale(HAL_DigitalHandle pwmPortHandle, int32_t squelchMask, int32_t* status) {
	...
	raspi::vmxIO->IsResourceActive(port->vmx_res_handle, isActive, status);
	if(isActive) {
    		/* In order that the new FrameOutputFilter takes effect, first Deactivate, then re-Activate the resource. */
    		raspi::vmxIO->DeactivateResource(port->vmx_res_handle, status);
		raspi::vmxIO->ActivateResource(port->vmx_res_handle, status);
	}
    }
}
