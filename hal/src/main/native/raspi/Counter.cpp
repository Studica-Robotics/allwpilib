...
static bool AllocateVMXPiInputCapture(
	...
	if (digPort->vmx_chan_info.capabilities & PWMCaptureInput) {
		...
		*status = RASPI_DIO_CHANNEL_COUNTER_INCOMPATIBILITY;
		return false;
	}
	...
	bool isActive = false;
	raspi::vmxIO->IsResourceActive(digPort->vmx_res_handle, isActive, status);
	if(isActive) {
		raspi::vmxIO->DeallocateResource(digPort->vmx_res_handle, status);
	...
	*status = 0;  // Clear status, which may have been set to an error above.
	...
	if (!raspi::vmxIO->ActivateSinglechannelResource(chan_info_copy,
...
static void DeallocateVMXPiInputCapture(std::shared_ptr<CounterResource> counter) {
	...
	if (!INVALID_VMX_RESOURCE_HANDLE(counter->vmx_res_handle)) {
		raspi::vmxIO->IsResourceActive(counter->vmx_res_handle, isActive,
			raspi::vmxError);
		if (isActive) {
			raspi::vmxIO->DeallocateResource(counter->vmx_res_handle, raspi::vmxError);
		}
...
static bool VerifyCounterInputSource(std::shared_ptr<CounterResource> counterResource,
	...
	if ((counterSourceType == HAL_HandleEnum::AnalogInput) ||
		...
		*status = RASPI_COUNTER_ANALOG_INPUTS_UNSUPPORTED;
		return false;
	}
    ...
    if (counterResource->hal_counter_mode != HAL_Counter_kExternalDirection) {
		...
		if (portUp->vmx_chan_info.capabilities & VMXChannelCapability::EncoderAInput) {
			...
			*status = RASPI_DIO_CHANNEL_ENCODER_INCOMPATIBILITY;
			return false;
		}
    }
...
static bool AllocReallocCounter(std::shared_ptr<CounterResource> counter, bool error_if_dual_extdirection_sources_not_present, int32_t *status) {
	...
	if (counter->hal_counter_mode != HAL_Counter_kExternalDirection) {
		...
		if (up_source_valid && down_source_valid) {
			*status = RASPI_COUNTER_DUAL_INPUTS_UNSUPPORTED;
			return false;
		}
		...
		if (!up_source_valid || !down_source_valid) {
			if (error_if_dual_extdirection_sources_not_present) {
				*status = RASPI_COUNTER_DUAL_INPUTS_REQUIRED;
			} else {
...
HAL_CounterHandle HAL_InitializeCounter(HAL_Counter_Mode mode, int32_t* index,
	...
	if (mode == HAL_Counter_Mode::HAL_Counter_kPulseLength) {
		*status = RASPI_COUNTER_PULSELENGTH_MODE_UNSUPPORTED;
...
void HAL_ResetCounter(HAL_CounterHandle counterHandle, int32_t* status) {
	...
	if (!INVALID_VMX_RESOURCE_HANDLE(counter->vmx_res_handle)) {
		...
		do { 
			raspi::vmxIO->InputCapture_Reset(counter->vmx_res_handle, status);
...
int32_t HAL_GetCounter(HAL_CounterHandle counterHandle, int32_t* status) {
	...
	if (!INVALID_VMX_RESOURCE_HANDLE(counter->vmx_res_handle)) {
		if (counter->hal_counter_mode == HAL_Counter_kSemiperiod) {
			...
			uint32_t ch1_count;
			uint32_t ch2_count;
			raspi::vmxIO->InputCapture_GetChannelCounts(counter->vmx_res_handle,
				ch1_count, ch2_count, status);
			counter_value = ch2_count;
		} else {
			raspi::vmxIO->InputCapture_GetCount(counter->vmx_res_handle, counter_value, status);
...
double HAL_GetCounterPeriod(HAL_CounterHandle counterHandle, int32_t* status) {
	...
	if (counter->hal_counter_mode != HAL_Counter_kSemiperiod) {
		*status = RASPI_COUNTER_SEMIPERIOD_MODE_REQUIRED;
		return 0.0;
	}
...
HAL_Bool HAL_GetCounterStopped(HAL_CounterHandle counterHandle,
	...
	if (!INVALID_VMX_RESOURCE_HANDLE(counter->vmx_res_handle)) {
		...
		bool active = false;
		raspi::vmxIO->InputCapture_InputStatus(counter->vmx_res_handle, forward_direction, active, status);
		raspi::ClearBoardCommErrorStatus(status);
		return active;
	} else if (counter->proxy_encoder_handle != HAL_kInvalidHandle) {
...
void HAL_SetCounterReverseDirection(HAL_CounterHandle counterHandle,
	...
	if (counter->hal_counter_mode != HAL_Counter_Mode::HAL_Counter_kExternalDirection) {
		*status = RASPI_COUNTER_EXT_DIRECTION_MODE_REQUIRED;
		return;
	}
