...
static bool AllocateVMXPiEncoder(
	...
	for (int i = 0; i < 2; i++) {
		...
		if (raspi::vmxIO->GetResourceFromRoutedChannel(channel_infos[i].index, res_handle, NULL)) {
			raspi::vmxIO->DeallocateResource(res_handle, NULL);
		}
	}
	const VMXChannelInfo *p_channel_infos = &channel_infos[0];
	if (!raspi::vmxIO->ActivateMultichannelResource(2, p_channel_infos,
...
static void DeallocateVMXPiEncoder(std::shared_ptr<EncoderResource> encoderResource,
	...
	if (encoderResource->index_interrupt_is_owned &&
		...
		raspi::vmxIO->DeactivateResource(encoderResource->index_interrupt_vmx_res_handle, raspi::vmxError);
	}

	isActive = false;
	raspi::vmxIO->IsResourceActive(encoderResource->vmx_res_handle, isActive,
			raspi::vmxError);
	if (isActive) {
		raspi::vmxIO->DeallocateResource(encoderResource->vmx_res_handle, raspi::vmxError);
	}
...
HAL_EncoderHandle HAL_InitializeEncoder(HAL_Handle digitalSourceHandleA,
	...
	if ((sourceAType == HAL_HandleEnum::AnalogInput) ||
		...
		(sourceBType == HAL_HandleEnum::AnalogTrigger)) {
		*status = RASPI_ENCODER_ANALOG_INPUTS_UNSUPPORTED;
...
int32_t HAL_GetEncoder(HAL_EncoderHandle encoderHandle, int32_t* status) {
	...
	if(!raspi::vmxIO->Encoder_GetCount(encoder->vmx_res_handle, counts, status)) {
...
void HAL_ResetEncoder(HAL_EncoderHandle encoderHandle, int32_t* status) {
	...
	do { 
		raspi::vmxIO->Encoder_Reset(encoder->vmx_res_handle, status);
	} while (raspi::RetryWriteOnBoardCommError(status, retry_count)); 	
}
...
double HAL_GetEncoderPeriod(HAL_EncoderHandle encoderHandle, int32_t* status) {
	...
	raspi::vmxIO->Encoder_GetLastPulsePeriodMicroseconds(encoder->vmx_res_handle, last_pulse_microseconds, status);
...
HAL_Bool HAL_GetEncoderStopped(HAL_EncoderHandle encoderHandle,
	...
	bool forward = true;
	bool active = false;
	raspi::vmxIO->InputCapture_InputStatus(encoder->vmx_res_handle, forward, active, status);
	raspi::ClearBoardCommErrorStatus(status);
	return !active;
}
...
void HAL_SetEncoderIndexSource(HAL_EncoderHandle encoderHandle,
	...
	if (!raspi::vmxIO->GetResourceHandle(VMXResourceType::Interrupt, vmx_interrupt_res_index, interrupt_resource_handle, status)) {
		return;
	}
	bool allocated = false;
	bool shared = false;
	if (!raspi::vmxIO->IsResourceAllocated(interrupt_resource_handle, allocated, shared, status)) {
		return;
	}
	...
	if (!allocated) {
		...
		if (!raspi::vmxIO->ActivateSinglechannelResource(interrupt_chan_info, &interrupt_config, interrupt_resource_handle, status)) {
			return;
		}
		...
		if (!raspi::vmxIO->GetResourceConfig(interrupt_resource_handle, p_cfg, status)) {
			return;
		}
	...
	if (!raspi::vmxIO->Encoder_SetResetSource(encoder->vmx_res_handle, interrupt_resource_handle, clear_on_level, clear_level_high, status)) {
		if (encoder->index_interrupt_is_owned) {
			raspi::vmxIO->DeactivateResource(interrupt_resource_handle, status);
			return;
		}
	}
