	...
	if (p_ledinfo->pwm_handle != HAL_kInvalidHandle) {
		...
		if (port) {
			while(!p_ledinfo->stop_update_thread) {
				int32_t status;
				raspi::vmxIO->LEDArray_Render(port->vmx_res_handle, &status);
...
HAL_AddressableLEDHandle HAL_InitializeAddressableLED(
  ...
  if (!raspi::vmxIO->ChannelSupportsCapability(vmx_chan_info.index, VMXChannelCapability::LEDArray_OneWire)) {
    *status = RASPI_PWM_CHANNEL_LEDARRAY_INCOMPATIBILITY;
    return HAL_kInvalidHandle;
  }
  ...
  if (!raspi::vmxIO->ActivateSinglechannelResource(vmx_chan_info, &new_port->ledarray_config, new_port->vmx_res_handle, status)) {
      int32_t temp_status;
      digitalChannelHandles->Free(pwmHandle, HAL_HandleEnum::PWM);
      HAL_InitializePWMPort(HAL_GetPort(wpi_digital_input_channel), nullptr, &temp_status);
      addressableLEDHandles->Free(handle);
      return HAL_kInvalidHandle;
  } else {
...
void HAL_FreeAddressableLED(HAL_AddressableLEDHandle handle) {
  ...
  if (led->buffer_handle) {
    raspi::vmxIO->LEDArrayBuffer_Delete(led->buffer_handle, &status);
    led->buffer_handle = 0;
  }
  ...
  HAL_InitializePWMPort(HAL_GetPort(led->wpi_digital_input_channel), nullptr, &status);
...
void HAL_SetAddressableLEDLength(HAL_AddressableLEDHandle handle,
  ...
  if (!raspi::vmxIO->LEDArray_Configure(vmxResource, port->ledarray_config, status)) {
    std::printf("HAL_SetAddressableLEDLength Error:  %s\n", HAL_GetErrorMessage(*status));
    return;
  } else {
    led->buffer_num_pixels = length;
    if (led->buffer_handle) {
      if (!raspi::vmxIO->LEDArrayBuffer_Delete(led->buffer_handle, status)) {
        std::printf("Error deleting VMXIO LEDArray Buffer:  %s\n", HAL_GetErrorMessage(*status));
      }
      led->buffer_handle = 0;
    }
    if (!raspi::vmxIO->LEDArrayBuffer_Create(length, led->buffer_handle, status)) {
      std::printf("Error creating VMXIO LEDArray Buffer:  %s\n", HAL_GetErrorMessage(*status));
    }
    if (!raspi::vmxIO->LEDArray_SetBuffer(vmxResource, led->buffer_handle, status)) {
...
void HAL_WriteAddressableLEDData(HAL_AddressableLEDHandle handle,
  ...
  for (int i = 0; i < num_leds_to_write; i++) {
    if (!raspi::vmxIO->LEDArrayBuffer_SetRGBValue(buffer_handle, i, data[i].r, data[i].g, data[i].b, status)) {
      return;
    }
  }
}
